...
import { actionTiles, bestPlan, movementOptions, type Action } from './allocate.ts';
import {
  canTarget,
  computeDamage,
  computeHeal,
...
// Raised from 30: the urban map is 17x22, so several turns go on approach.
...
export function createBattle(
  ...
  const mk = (defs: CharacterDef[], side: Side, spawns: Pos[]): Unit[] =>
    defs.map((def, i) => ({
      def,
      hp: def.maxHp,
      atkBuff: 0,
  ...
  const state: BattleState = {
    map,
    units: [
      ...mk(playerDefs, 'player', map.playerSpawns),
      ...mk(enemyDefs, 'enemy', map.enemySpawns),
    ],
    turn: 1,
    phase: 'player',
    dice: [],
...
export function commitAction(
  ...
  // Range and sight are measured from where the unit will END UP, and both are
  // checked before any mutation so a rejected action cannot leave it half-moved.
  if (!canTarget(ability, movePos, target, s.map)) {
    return `${unit.def.name} has no line of sight to that tile.`;
  }

  if (!samePos(movePos, unit.pos)) {
    // A dash is part of the ability, so it is allowed even after a normal move.
    const legal = actionTiles(unit, ability, s.units, s.map).some((t) => samePos(t, movePos));
    if (!legal) {
      return unit.hasMoved && !ability.dash
        ? `${unit.def.name} has already moved this turn.`
        : `${unit.def.name} cannot reach that tile.`;
    }
    s.log.push({ t: 'move', unit: unit.def.name, from: { ...unit.pos }, to: { ...movePos } });
    unit.pos = { ...movePos };
    unit.hasMoved = true;
  }

  for (const i of diceIndices) s.diceSpent[i] = true;
  // Acting ends the character's turn: the action is spent and so is any
  // movement it had left. Moving alone leaves the action available.
  unit.hasActed = true;
  unit.hasMoved = true;
...
const canReach = (s: BattleState, unit: Unit, pos: Pos): boolean =>
  movementOptions(unit, s.units, s.map).some((t) => samePos(t, pos));

/**
 * Reposition without acting. Movement never costs dice and never costs the
 * character's action -- after moving it can still cast. Once per turn, and
 * unavailable after the character has acted, since acting ends its turn.
 */
export function commitMove(s: BattleState, unit: Unit, pos: Pos): string | null {
  if (s.outcome !== 'ongoing') return 'The battle is over.';
  if (unit.side !== s.phase) return 'It is not that side’s phase.';
  if (unit.hasMoved) return `${unit.def.name} has already moved this turn.`;
  if (samePos(pos, unit.pos)) return null;
  if (!canReach(s, unit, pos)) return `${unit.def.name} cannot reach that tile.`;

  s.log.push({ t: 'move', unit: unit.def.name, from: { ...unit.pos }, to: { ...pos } });
  unit.pos = { ...pos };
  unit.hasMoved = true;
  return null;
}
  ...
  switch (ability.kind) {
    case 'attack': {
      for (const target of unitsHit(ability, centre, foes)) {
        const dmg = computeDamage(source, ability, target, s.map);
...
/** Player-side auto-battle: allocate the dice pool, then advance stragglers. */
function nextDiceStep(s: BattleState, team: Unit[], foes: Unit[]): AiStep | null {
  if (!s.ai) s.ai = { plan: bestPlan(team, s.dice, foes, s.map).actions, index: 0 };
  ...
  while (s.ai.index < s.ai.plan.length) {
    ...
    if (commitAction(s, action.unit, action.ability, indices, action.movePos, action.target)) continue;
    return { unit: action.unit, kind: 'act', ability: action.ability, target: action.target };
  }
  return advanceOne(s, team, foes);
}

/**
 * Enemy side, one character at a time: land anything telegraphed last turn,
 * then take the best available action, then close the distance.
 */
function nextEnemyStep(s: BattleState, team: Unit[], foes: Unit[]): AiStep | null {
  for (const u of team) {
    ...
    u.hasActed = true;
    u.hasMoved = true;
    // Resolves on the tile announced last turn, not on wherever the player is
    // standing now -- that is the whole point of the warning.
  ...
  for (const u of team) {
    ...
    const { ability, movePos, target } = choice;
    if (!samePos(movePos, u.pos)) {
      s.log.push({ t: 'move', unit: u.def.name, from: { ...u.pos }, to: { ...movePos } });
      u.pos = { ...movePos };
    }
    u.hasMoved = true;
    u.hasActed = true;
...
function chooseEnemyAction(s: BattleState, unit: Unit, foes: Unit[]): EnemyChoice | null {
  ...
  for (const ability of ordered) {
    ...
    const tiles = actionTiles(unit, ability, s.units, s.map);
    const centres = (ability.kind === 'attack' ? foes : allies).map((u) => u.pos);

    for (const from of tiles) {
      for (const centre of centres) {
        if (!canTarget(ability, from, centre, s.map)) continue;
        const score = scoreAction(unit, ability, centre, allies, foes, s.map);
        if (score <= 0) continue;
        if (!best || priority > best.priority || score > best.score) {
          best = { ability, movePos: from, target: centre, priority, score };
        }
      }
    }
  }

  return best ? { ability: best.ability, movePos: best.movePos, target: best.target } : null;
}
...
export function runAiPhase(s: BattleState): void {
  ...
  while (nextAiStep(s) && guard++ < 64) {
    /* each call performs one action or one move */
  }
}

/**
 * Move a single unit that is out of the fight toward it, or return null if
 * everyone is either engaged or has already moved.
 *
 * Runs regardless of who acted: now that every character has an always-usable
 * basic, a unit can self-buff forever without closing, and gating advance on
 * "did nothing" left whole teams standing at spawn until the turn cap.
 */
function advanceOne(s: BattleState, team: Unit[], foes: Unit[]): AiStep | null {
  if (foes.length === 0) return null;

  // One Dijkstra sweep from every enemy gives true walking distance, so units
  // route around buildings instead of hill-climbing into them.
  const field = distanceField(s.map, foes.map((f) => f.pos));
  const living = s.units.filter(alive);

  for (const u of team) {
    if (!alive(u) || u.hasMoved) continue;

    // Already in the fight: hold ground rather than shuffling into more danger.
    const reach = Math.max(...u.def.abilities.map((a) => a.range + (a.dash ?? 0)));
    if (foes.some((f) => manhattan(u.pos, f.pos) <= reach)) continue;

    let best = u.pos;
    let bestScore = Infinity;
    for (const tile of movementOptions(u, living, s.map)) {
      const walk = field.get(key(tile));
      if (walk === undefined) continue;
      const score = walk * 10 - TERRAIN[terrainAt(s.map, tile)].defBonus / 10;
      if (score < bestScore) {
        bestScore = score;
        best = tile;
      }
    ...
    if (!samePos(best, u.pos)) {
      commitMove(s, u, best);
      return { unit: u, kind: 'move' };
    }
    // Nowhere better to stand; mark it done so the phase can finish.
    u.hasMoved = true;
  }
  return null;
}
...
export function simulateBattle(
  ...
  const s = createBattle(map, playerDefs, enemyDefs, seed);
  let guard = 0;
